...
class PublicRecipeApiTests(TestCase):
    ...
    def test_auth_required(self):
        """Test auth is required to call the recipe API."""
...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_update_user_returns_error(self):
        """Test changing user of a recipe returns an error."""
    ...
    def test_recipe_other_users_recipe_error(self):
        """Test trying to delete other user's recipe returns an error."""
        new_user = create_user(email='user2@example.com', password='test123')
    ...
    def test_create_recipe_with_existing_tag(self):
        ...
        tag_risoto = Tag.objects.create(user=self.user, name='Risoto')
        payload = {
            'title': 'Risoto de camarão',
            'time_minutes': 30,
            'price': Decimal('5.99'),
            'tags': [{'name': 'Camarão'}, {'name': 'Risoto'}],
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
        ...
        recipe = recipes[0]
        self.assertEqual(recipe.tags.count(), 2)
        self.assertIn(tag_risoto, recipe.tags.all())
        for tag in payload['tags']:
            exists = recipe.tags.filter(
                name=tag['name'],
                user=self.user,
    ...
    def test_create_tag_on_update(self):
        ...
        payload = {'tags': [{'name': 'Camarão'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        new_tag = Tag.objects.get(user=self.user, name='Camarão')
        self.assertIn(new_tag, recipe.tags.all())

    def test_update_recipe_assign_tag(self):
        """Test assigning an existing tag when updatind a recipe."""
        tag_risoto = Tag.objects.create(user=self.user, name='Risoto')
        recipe = create_recipe(user=self.user)
        recipe.tags.add(tag_risoto)

        tag_camarao = Tag.objects.create(user=self.user, name='Camarão')
        payload = {'tags': [{'name': 'Camarão'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertIn(tag_camarao, recipe.tags.all())
        self.assertNotIn(tag_risoto, recipe.tags.all())
    ...
    def test_create_recipe_with_new_ingredient(self):
        ...
        payload = {
            ...
            'time_minutes': 60,
            'price': Decimal('5.99'),
            'ingredients': [{'name': 'Camarão'}, {'name': 'Arroz'}],
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
    ...
    def test_create_recipe_with_existing_ingredient(self):
        ...
        ingredient = Ingredient.objects.create(user=self.user, name='Linguiça')
        payload = {
            'title': 'Risoto de Linguiça',
            'time_minutes': 60,
            'price': '4.99',
            'ingredients': [{'name': 'Linguiça'}, {'name': 'Alho Poró'}],
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
    ...
    def test_create_ingredient_on_update(self):
        ...
        payload = {'ingredients': [{'name': 'Mignon'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        new_ingredient = Ingredient.objects.get(user=self.user, name='Mignon')
        self.assertIn(new_ingredient, recipe.ingredients.all())

    def test_update_recipe_assign_ingredient(self):
        """Test assigning an existing ingredient when updating a recipe."""
        ingredient1 = Ingredient.objects.create(user=self.user, name='Arroz')
        recipe = create_recipe(user=self.user)
        recipe.ingredients.add(ingredient1)

        ingredient2 = Ingredient.objects.create(user=self.user, name='Cebola')
        payload = {'ingredients': [{'name': 'Cebola'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')
    ...
    def test_filter_by_tags(self):
        ...
        tag1 = Tag.objects.create(user=self.user, name="Brasileira")
        tag2 = Tag.objects.create(user=self.user, name="Italiana")
        r1.tags.add(tag1)
        r2.tags.add(tag2)
